...
const Index = () => {
    ...
    const { refetch, isFetching } =
        useGet({ key: ['itm_specific'], 
            fetch: specific({ table: 'tbl_physical_count_ras', data: JSON.stringify({ id: id ?? null, physical_count_id: data.physical_count_id }) }), 
    ...
    const { mutate: assign } =
        usePost({ fetch: save,
            onSuccess: res => {
                ...
                else { successToast(res.message, 3000, navigate('/ras', { replace: true })); } 
            } 
        });
