...
const Items = () => {
    ...
    return (
        <Box sx= {{ overflowY: 'scroll', '&::-webkit-scrollbar': { display: 'none' } }}>
            { list?.length > 0 ?
                list?.map((data, index) => (
                    <Stack key= { index } sx= { card } component= { Link } to= { `/attendees/form/update/${data.user_id}` }>
                ...
                )) : <Stack sx= { card }><Typography variant= "body2" sx= {{ textAlign: 'center', width: '100%' }}>No employee/s found!</Typography></Stack> }
        </Box>
    );
}
