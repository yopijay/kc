...
const Index = () => {
    ...
    const { mutate: counts } = usePost({ fetch: count, 
        onSuccess: data => { setValue('total_items', getValues().type === 'annual' ? data : (getValues().brands).length > 0 ? (getValues().brands).reduce((a, b) => a +b.items ?? 0, 0) : 0); }
    });
            ...
            <Box sx= { card }>
                <form autoComplete= "off">
                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" spacing= { 2 }>
                        <ThemeProvider theme= { theme(input) }><Info fetching= { isFetching } counttype= { counttype } setCounttype= { setCounttype } counts= { counts } /></ThemeProvider>
                    ...
                    <Grid item xs= { 12 } sm= { 3 } lg= { 2 }>
                        <Box sx= { btntxt } onClick= { handleSubmit(data => {
                            ...
                            if(!(errors.length > 0)) {
                                ...
                                else { updating({ table: 'tbl_physical_count', data: data }); }
                            }
