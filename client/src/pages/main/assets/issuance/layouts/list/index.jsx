...
const Index = () => {
    ...
    useEffect(() => {
        ...
        record({ table: 'tbl_assign_asset', data: data });
                        ...
                        <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" sx= {{ flexGrow: 1 }} spacing= { 1 }>
                            { data.user_level === 'superadmin' ?
                                <input type= "file" name= "upload-file" id= "upload-file" style= {{ width: '0.1px', height: '0.1px', opacity: 0, overflow: 'hidden', position: 'absolute', zIndex: -1 }}
                                    onChange= { async e => { 
                                        uploadfile({ table: 'tbl_assign_asset', data: { json: await importfromexcel(e), id: localStorage.getItem('token') } }); e.target.value = '' } } /> : ''}
                            ...
                            <Typography component= { Link } to= "/assets/asset-issuance/form/new" sx= { btntxt }>New Issuance</Typography>
                        </Stack>
