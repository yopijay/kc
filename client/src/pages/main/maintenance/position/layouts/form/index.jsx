...
const Index = () => {
    ...
    const { isFetching, refetch } =  
        useGet({ key: ['pst_specific'], fetch: specific({ table: 'tbl_position', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false}, 
            onSuccess: (data) => { 
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px' }} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                <Typography variant= "h6" sx= {{ fontFamily: 'Boldstrom', color: '#3C4048' }}>{ type } Position</Typography>
                <Typography sx= { btnicon } component= { Link } to= "/maintenance/position" ><FontAwesomeIcon icon= { faChevronLeft }/></Typography>
            </Stack>
