...
const Index = () => {
    ...
    useEffect(() => { record({ table: 'tbl_company', data: { category: category, orderby: orderby, searchtxt: searchtxt, limit: 15 } }); }, [ record, category, orderby, searchtxt ]);
                ...
                <Stack direction= "row" justifyContent= "space-between" alignItems= "flex-start" spacing= { 1 }>
                    ...
                    <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" spacing= { 1 }>
                        <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" sx= {{ flexGrow: 1 }} spacing= { 1 }>
                            ...
                            <Typography 
                                onClick= { () => { 
                                    if(data.user_level === 'superadmin') { original({ table: 'tbl_company', type: 'original', condition: { orderby: orderby, category: category } }); } 
                                    formatted({ table: 'tbl_company', type: 'formatted', condition: { orderby: orderby, category: category, searchtxt: searchtxt } }); }} sx= { btnexport }>
