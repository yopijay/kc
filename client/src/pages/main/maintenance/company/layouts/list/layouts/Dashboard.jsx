...
const Dashboard = () => {
    ...
    useEffect(() => { dash({ table: 'tbl_company', data: {} }); }, [ dash ]);
        ...
        <Grid container direction= "row" justifyContent= "flex-start" alignItems= "stretch" sx= {{ padding: '5px 0' }}>
            <Grid item xs= { 4 } sm= { 6 } sx= {{ padding: { xs: '0 5px 0 0' } }}>
                <Stack direction= "column" justifyContent= "flex-start" alignItems= "flex-start" sx= { card }>
                    { !isLoading ? <Typography variant= "h5" sx= {{ fontFamily: 'Boldstrom' }} color= "text.secondary">{ `${count.total < 10 ? '0' : ''}${count.total}` }</Typography> : 
            ...
            <Grid item xs= { 4 } sm= { 3 } sx= {{ padding: '0 5px' }}>
                <Stack direction= "column" justifyContent= "flex-start" alignItems= "flex-start" sx= { card }>
                    { !isLoading ? <Typography variant= "h5" sx= {{ fontFamily: 'Boldstrom' }} color= "text.secondary">{ `${count.active < 10 ? '0' : ''}${count.active}` }</Typography> : 
                        <Skeleton variant= "text" sx= {{ width: '50px', fontSize: '1rem' }} /> }
                    <Typography variant= "body1" sx= { label }>ACTIVE</Typography>
                </Stack>
            </Grid>
            <Grid item xs= { 4 } sm= { 3 } sx= {{ padding: '0 5px' }}>
                <Stack direction= "column" justifyContent= "flex-start" alignItems= "flex-start" sx= { card }>
                    { !isLoading ? <Typography variant= "h5" sx= {{ fontFamily: 'Boldstrom' }} color= "text.secondary">{ `${count?.inactive < 10 ? '0' : ''}${count?.inactive}` }</Typography> : 
                        <Skeleton variant= "text" sx= {{ width: '50px', fontSize: '1rem' }} /> }
                    <Typography variant= "body1" sx= { label }>INACTIVE</Typography>
                </Stack>
            </Grid>
        </Grid>
    );
