...
const Index = () => {
    ...
    useEffect(() => record({ table: 'tbl_department', data: { category: category, orderby: orderby, searchtxt: searchtxt }}), [ record, category, orderby, searchtxt ]);
                        ...
                        <Box sx= { search }>
                            ...
                            <TextField variant= "standard" size= "small" fullWidth InputProps= {{ disableUnderline: true }} placeholder= "Search..." sx= {{ padding: '5px 0 0 0' }}
                                onChange= { e => { 
                                    ...
                                    find({ table: 'tbl_department', data: { condition: e.target.value !== '' ? (e.target.value).toUpperCase() : e.target.value,
                        ...
                        <Stack direction= "row" justifyContent= "flex-end" alignItems= "center" sx= {{ flexGrow: 1 }} spacing= { 1 }>
                            { data.user_level === 'superadmin' ?
                                <input type= "file" name= "upload-file" id= "upload-file" style= {{ width: '0.1px', height: '0.1px', opacity: 0, overflow: 'hidden', position: 'absolute', zIndex: -1 }}
                                    onChange= { async e => { uploadfile({ table: 'tb_department', data: { json: await importfromexcel(e), id: atob(localStorage.getItem('token')) } }); e.target.value = '' } } /> : ''}
                            ...
                            <Typography 
                                onClick= { () => { 
                                    if(data.user_level === 'superadmin') { original({ table: 'tbl_department', type: 'original', condition: { orderby: orderby, category: category } }); } 
                                    formatted({ table: 'tbl_department', type: 'formatted', condition: { orderby: orderby, category: category, searchtxt: searchtxt } }); }} sx= { btnexport }>
