...
const Index = () => {
    ...
    const { isFetching, refetch } =  
        useGet({ key: ['trckr_specific'], fetch: specific({ table: 'tbl_tracker', id: id ?? null }), options: { enabled: type !== 'new', refetchOnWindowFocus: false}, 
            onSuccess: (data) => { 
    ...
    const { mutate: saving } = 
        usePost({ fetch: save, 
            onSuccess: (data) => {
                ...
                else { successToast(data.message, 3000, navigate('/maintenance/tracker', { replace: true })); } 
            } 
        });
    ...
    return (
        <Stack direction= "column" justifyContent= "flex-start" alignItems= "stretch" sx= {{ width: '100%', height: '100%', paddingBottom: '20px' }} spacing= { 3 }>
            <Stack direction= "row" justifyContent= "space-between" alignItems= "center">
                <Typography variant= "h6" sx= {{ fontFamily: 'Boldstrom', color: '#3C4048' }}>{ type } Tracker</Typography>
                <Typography sx= { btnicon } component= { Link } to= "/maintenance/tracker" ><FontAwesomeIcon icon= { faChevronLeft }/></Typography>
            </Stack>
                ...
                <Grid container direction= "row" justifyContent= "flex-end" alignItems= "center">
                    <Grid item xs= { 12 } sm= { 3 } lg= { 2 }>
                        <Box sx= { btntxt } onClick= { handleSubmit(data => {
                            ...
                            if(type === 'new') { saving({ table: 'tbl_tracker', data: data }); }
                            else { updating({ table: 'tbl_tracker', data: data }); }
                        }) }>Save</Box>
                    </Grid>
                </Grid> : '' }
        </Stack>
