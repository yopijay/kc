...
class Company {
    ...
    list = async (data) => {
        return (await new Builder(`tbl_company AS cmp`)
                                        ...
                                        .condition(`${data.searchtxt !== '' || data.searchtxt !== undefined ? 
                                                                `WHERE cmp.series_no LIKE '%${(data.searchtxt).toUpperCase()}%' OR cmp.name LIKE '%${(data.searchtxt).toUpperCase()}%'` : '' }
                                                            ORDER BY cmp.${data.orderby !== '' || data.orderby !== undefined ? data.orderby : 'date_created'} 
                                                            ${data.sort !== '' || data.sort !== undefined ? (data.sort).toUpperCase() : 'DESC'}`)
                                        .build()).rows;
    }
