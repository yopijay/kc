...
class Keyboard {
    formatted = async (data) => {
        return (await new Builder(`tbl_assets AS assts`)
                                        .select(`assts.series_no AS "Series No.", info.serial_no AS "Serial no.", assts.asset_tag AS "Asset Tag", info.brand AS "Brand", info.model AS "Model",
                                                        info.color AS "Color", info.interface AS "Interface", info.no_of_keys AS "No. of Keys", info.date_purchased AS "Date purchased", 
    ...
    original = async (data) => {
        return (await new Builder(`tbl_assets AS assts`)
                                        .select(`assts.id, assts.series_no, assts.category_id, assts.sub_category_id, info.serial_no, assts.asset_tag, info.brand, info.model,
                                                        info.color, info.interface, info.no_of_keys, info.date_purchased, info.warranty, assts.is_released, assts.status, 
    ...
    update = async (data) => {
        ...
        if(Global.compare(assts.no_of_keys, data.no_of_keys)) {
            _audit.push({ series_no: Global.randomizer(7), table_name: 'tbl_assets', item_id: assts.id, 
                                    field: 'no_of_keys', previous: assts.no_of_keys, current: data.no_of_keys, action: 'update', user_id: data.updated_by, date: date });
        }
        ...
        await new Builder(`tbl_assets_info`)
                            .update(`serial_no= ${data.serial_no !== '' || data.serial_no !== null ? `'${(data.serial_no).toUpperCase()}'` : null},
                                            ...
                                            interface= '${data.interface}', no_of_keys= ${data.no_of_keys !== '' || data.no_of_keys !== null ? data.no_of_keys : null},
