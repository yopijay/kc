    ...
    formatted = async (data) => {
        return (await new Builder(`tbl_assets AS assts`)
                                        .select(`assts.series_no AS "Series No.", info.serial_no AS "Serial no.", assts.asset_tag AS "Asset Tag", info.brand AS "Brand", info.model AS "Model", 
                                                        info.os AS "Operating System", info.processor AS "Processor", info.video_card AS "Video Card", info.resolution AS "Resolution", 
                                                        info.ram AS "RAM", info.hdd AS "HDD", info.ssd AS "SSD", info.input_connectivity AS "Input Connectivity", info.date_purchased AS "Date purchased", 
    ...
    original = async (data) => {
        return (await new Builder(`tbl_assets AS assts`)
                                        .select(`assts.id, assts.series_no, assts.category_id, assts.sub_category_id, info.serial_no, assts.asset_tag, info.brand, info.model, info.os, info.processor,
                                                        info.video_card, info.resolution, info.ram, info.hdd, info.ssd, info.input_connectivity, info.date_purchased, info.warranty, assts.is_released, assts.status, 
