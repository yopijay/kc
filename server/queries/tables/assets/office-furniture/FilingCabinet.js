...
class FilingCabinet {
    ...
    update = async (data) => {
        ...
        if(Global.compare(assts.no_of_drawers, data.no_of_drawers)) {
            _audit.push({ series_no: Global.randomizer(7), table_name: 'tbl_assets', item_id: assts.id, 
                                    field: 'no_of_drawers', previous: assts.no_of_drawers, current: data.no_of_drawers, action: 'update', user_id: data.updated_by, date: date });
        }
        ...
        await new Builder(`tbl_assets_info`)
                            .update(`brand= ${data.brand !== '' || data.brand !== null ? `'${(data.brand).toUpperCase()}'` : null},
                                            ...
                                            no_of_drawers= ${data.no_of_drawers !== '' || data.no_of_drawers !== null ? data.no_of_drawers : null},
