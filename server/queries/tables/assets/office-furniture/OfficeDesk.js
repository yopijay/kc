...
class OfficeDesk {
    ...
    update = async (data) => {
        ...
        if(Global.compare(assts.with_sidetable, data.with_sidetable)) {
            _audit.push({ series_no: Global.randomizer(7), table_name: 'tbl_assets', item_id: assts.id, 
                                    field: 'with_sidetable', previous: assts.with_sidetable, current: data.with_sidetable, action: 'update', user_id: data.updated_by, date: date });
        }
        ...
        await new Builder(`tbl_assets_info`)
                            .update(`brand= ${data.brand !== '' || data.brand !== null ? `'${(data.brand).toUpperCase()}'` : null}, 
                                            ...
                                            appearance= '${data.appearance}', with_sidetable= '${data.with_sidetable}'`)
